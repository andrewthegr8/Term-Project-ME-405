Software (MicroPython)
=======================

The program design for this project consists of several tasks which 
are run by a prioritized scheduler (:mod:`cotask`). Each task is a generator function,
and many are configured as finite state machines. Tasks communicate using 
``share`` and ``queue`` objects from :mod:`task_share`, which, along with the scheduler, are 
adapted from code developed by Dr. John Ridgley. The original code for the
scheduler and inter-task communication objects
can be found in the `ME405-Support <https://github.com/spluttflob/ME405-Support>`_ repository. 

The software documentation is broken up into 3 sections:

* :doc:`main-program` - Documentation for the tasks that provide high level functionality
* :doc:`helper-classes` - Documentation for specalized object types developed to preform specific functions in this project
* :doc:`drivers` - Documentation for driver classes developed to interface with hardware

.. toctree::
   :maxdepth: 2
   :caption: Software Modules

   main-program
   helper-classes
   drivers